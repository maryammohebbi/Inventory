...
function CategoryForm() {
  ...
  return (
    <div className='section'>
        <section>
            <div className={`${!open && "hidden"} mb-5`}>
                ...
                <form onSubmit={handleSubmit} className= "bg-slate-700 p-5 flex flex-col gap-y-6 rounded-lg"> 
                    ...
                    <div className="flex gap-x-2">
                        <button 
                            ...
                            className="flex-1 border border-slate-500 py-3 px-7 text-xs rounded text-slate-300 bg-transparent">Cancel</button>
            ...
            <button 
                onClick={()=> setOpen(!open)}
                className={`${open && "hidden"} text-slate-500 text-lg mb-5`}>Add New Category?</button>
        </section>
    </div>
  )
}
